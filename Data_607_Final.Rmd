...
counts_and_reports_by_category <- incidents_with_types %>%
...
counts_and_reports_by_category <- counts_and_reports_by_category %>%
  rename(year = Incident.Year)

year_pop_counts_categories <- full_join(sf_pop, counts_and_reports_by_category, by = 'year')

counts_and_reports_by_category_per_100k <- year_pop_counts_categories %>%
  group_by(year, Incident.Category) %>%
  mutate(incidents_per_100k = 100000 * count_incidents/Population, reports_per_100k = 100000 * count_reports/Population) %>%
  arrange(year, desc(count_reports))

counts_and_reports_by_category_per_100k
```
